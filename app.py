# -*- coding: utf-8 -*-
"""
Streamlit – Formulário para gerar Relatório Técnico (com LOGO)
...
- Exporta PDF (ReportLab, puro Python) e DOCX (python-docx) com LOGO.
...
Observações:
- PDF via LaTeX exigiria TeX instalado no servidor. Aqui usamos ReportLab (sem dependências externas).
- O LOGO é opcional. Se não subir arquivo, o PDF/DOCX sai sem logo.
"""
...
st.caption("Preencha os campos e exporte em PDF ou DOCX. Você também pode salvar um rascunho em JSON.")
...
# ====== Logo e rascunhos (upload/download) ======
    ...
    imported: Optional[Relatorio] = None
    if up is not None:
        try:
            data = json.load(up)
...
with st.form("form-relatorio"):
    ...
    # Ajusta lista de autores ao tamanho desejado
...
try:
    pdf_bytes = build_pdf(rel, st.session_state.get('logo_bytes'), logo_width_cm)
...
try:
    docx_bytes = build_docx(rel, st.session_state.get('logo_bytes'), logo_width_cm)
...
# Salvar rascunho JSON do estado atual
st.download_button(
...
st.caption("Dica: personalize os rótulos/ordem das seções direto no código para criar templates por cliente (ex.: OGMP, InSAR, Portos, MDA, etc.).")
